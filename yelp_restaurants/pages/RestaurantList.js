...
const RestaurantList = (props) =>{
    ...
    function searchRestaurant(search){
        const filteredRestaurants = originalList.filter(restaurant =>{
            const userInput = search.toUpperCase();
            const restaurantName = restaurant.name.toUpperCase();

            return restaurantName.indexOf(userInput) > -1;
        })
    ...
    return(
        <SafeAreaView style={{flex:1}}>
            <View style={{flex:1}}>
                <View>
                    <Text style={{fontSize:30, margin:5, fontWeight: 'bold'}}>Restaurants in {selectedCity}</Text>
                    <SearchBar 
